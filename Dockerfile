...
RUN cmake --build /opt/lupine/build --parallel \
      --target lupine_cuda_client lupine_cudart_client lupine_cublas_client lupine_cufft_client lupine_nvml_client lupine_hip_client
...
LABEL org.opencontainers.image.description="LUPINE client runtime with CUDA driver, CUDA runtime, cuBLAS, cuFFT, NVML, and HIP shims"
...
RUN ln -sf /opt/lupine/lib/libcuda.so.1 /opt/lupine/lib/libcuda.so \
    ...
    && ln -sf "$(basename /opt/lupine/lib/libcufft.so.*)" /opt/lupine/lib/libcufft.so \
