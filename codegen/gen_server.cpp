...
int handle_cuDeviceGetUuid(conn_t *conn)
{
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuDeviceGetUuid(uuid, dev);
...
int handle_cuCtxCreate_v2(conn_t *conn)
{
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuCtxCreate_v2(&pctx, flags, dev);
...
int handle_cuCtxCreate_v3(conn_t *conn)
{
    CUcontext pctx;
    int numParams;
    CUexecAffinityParam* paramsArray = nullptr;
    unsigned int flags;
    CUdevice dev;
    int request_id;
    CUresult lupine_intercept_result;
    ...
    if ((numParams * sizeof(CUexecAffinityParam) != 0 && paramsArray == nullptr) ||
        rpc_read(conn, &flags, sizeof(unsigned int)) < 0 ||
        rpc_read(conn, &dev, sizeof(CUdevice)) < 0 ||
        false)
        goto ERROR_0;
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuCtxCreate_v3(&pctx, (numParams * sizeof(CUexecAffinityParam) == 0 ? nullptr : paramsArray), numParams, flags, dev);

    if (rpc_write_start_response(conn, request_id) < 0 ||
        rpc_write(conn, &pctx, sizeof(CUcontext)) < 0 ||
        (numParams * sizeof(CUexecAffinityParam) != 0 && rpc_write(conn, paramsArray, numParams * sizeof(CUexecAffinityParam)) < 0) ||
        rpc_write(conn, &lupine_intercept_result, sizeof(CUresult)) < 0 ||
        rpc_write_end(conn) < 0)
        goto ERROR_0;
...
int handle_cuMemcpyDtoDAsync_v2(conn_t *conn)
{
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuMemcpyDtoDAsync_v2(dstDevice, srcDevice, ByteCount, hStream);
...
int handle_cuMemsetD2D8Async(conn_t *conn)
{
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuMemsetD2D8Async(dstDevice, dstPitch, uc, Width, Height, hStream);
...
int handle_cuMemsetD2D16Async(conn_t *conn)
{
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuMemsetD2D16Async(dstDevice, dstPitch, us, Width, Height, hStream);
...
int handle_cuMemsetD2D32Async(conn_t *conn)
{
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuMemsetD2D32Async(dstDevice, dstPitch, ui, Width, Height, hStream);
...
int handle_cuMemPrefetchAsync(conn_t *conn)
{
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuMemPrefetchAsync(devPtr, count, dstDevice, hStream);
...
int handle_cuStreamUpdateCaptureDependencies(conn_t *conn)
{
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuStreamUpdateCaptureDependencies(hStream, &dependencies, numDependencies, flags);
...
int handle_cuStreamBatchMemOp_v2(conn_t *conn)
{
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuStreamBatchMemOp_v2(stream, count, &paramArray, flags);
...
int handle_cuGraphAddDependencies(conn_t *conn)
{
    ...
    if (
        ...
        false)
        goto ERROR_0;
    from_size = numDependencies * sizeof(const CUgraphNode);
    from = (CUgraphNode*)malloc(from_size);
    if (from_size != 0 && from == nullptr)
        goto ERROR_0;
    if(
        (from_size != 0 && rpc_read(conn, from, from_size) < 0) ||
        false)
        goto ERROR_0;
    to_size = numDependencies * sizeof(const CUgraphNode);
    to = (CUgraphNode*)malloc(to_size);
    if (to_size != 0 && to == nullptr)
        goto ERROR_0;
    if(
        (to_size != 0 && rpc_read(conn, to, to_size) < 0) ||
        false)
        goto ERROR_0;
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuGraphAddDependencies(hGraph, (numDependencies * sizeof(const CUgraphNode) == 0 ? nullptr : from), (numDependencies * sizeof(const CUgraphNode) == 0 ? nullptr : to), numDependencies);
...
int handle_cuGraphRemoveDependencies(conn_t *conn)
{
    ...
    if (request_id < 0)
        goto ERROR_0;
    lupine_intercept_result = cuGraphRemoveDependencies(hGraph, (numDependencies * sizeof(const CUgraphNode) == 0 ? nullptr : from), (numDependencies * sizeof(const CUgraphNode) == 0 ? nullptr : to), numDependencies);
