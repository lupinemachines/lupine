...
the device (a cuBLAS `alpha`, `beta`, or dot-product `result`). The call's first
parameter is the handle whose mode decides: the generated client asks
`scalar_on_host(handle)` and sends the value in host mode or the address in
device mode, with the width leading on the wire so the server can tell which.
`SEND_RECV SCALAR` brings a host value back (`cublasSrotg`). A `void` scalar
carries its width as `SIZE:<expr>`, a C++ expression the client evaluates over
the call's arguments (`SIZE:data_type_width(resultType)`); a typed scalar wider
than its pointee spells that out the same way (`SIZE:5*sizeof(float)`).
