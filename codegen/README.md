...
CUDA's stream aliases are generated by `cuda_streams.py` from the SDK's
`__CUDA_API_PTSZ` / `__CUDA_API_PTDS` declarations. The same registry supplies
exports, lookup entries, version selection, and synchronous per-thread RPCs.
Explicit-stream aliases translate only stream zero (including nested launch and
upload streams); implicit-stream aliases retain the native synchronous API and
its host-blocking behavior. Keep ABI changes in `STREAM_VERSIONS`, and toolkit
availability on the underlying annotation's `@guard`. Client-only compatibility
declarations must stay out of the server header so older SDK adapters keep their
