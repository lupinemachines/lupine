...
// LoadLibrary/dlopen, so no HIP SDK is required to *build* lupine. When the AMD
// backend is enabled it simply binds at runtime if a ROCm install is present.
// This also keeps HIP client/server builds independent of both the ROCm SDK and
// the CUDA/NVML components.
//
...
// are composed of (hipUUID, hipDeviceArch_t). The v1 surface marshals the
// fixed-layout property struct as a whole; generated code touches only its two
// process-local pointer fields to scrub them at the wire boundary.
//
...
typedef struct hipDeviceProp_tR0600 {
  ...
  int maxThreadsPerMultiProcessor;   ///< Max resident threads per
                                     ///< multi-processor.
  ...
  int multiGpuBoardGroupID;  ///< Group ID for devices on the same multiboard
                             ///< GPU.
  ...
  int canUseHostPointerForRegisteredMem; ///< Host-registered memory at same
                                         ///< address.
  int cooperativeLaunch;                 ///< Supports cooperative launch.
  int cooperativeMultiDeviceLaunch;      ///< Supports multi-device cooperative
                                         ///< launch.
  size_t sharedMemPerBlockOptin;         ///< Opt-in shared mem per block.
  int pageableMemoryAccessUsesHostPageTables; ///< Pageable access via host page
                                              ///< tables.
  ...
  size_t
      reservedSharedMemPerBlock; ///< Shared mem reserved by driver per block.
  ...
  unsigned int
      gpuDirectRDMAFlushWritesOptions; ///< hipFlushGPUDirectRDMAWritesOptions
                                       ///< bitmask.
  int gpuDirectRDMAWritesOrdering; ///< hipGPUDirectRDMAWritesOrdering value.
  unsigned int
      memoryPoolSupportedHandleTypes;   ///< Mempool IPC handle-type bitmask.
  int deferredMappingHipArraySupported; ///< Supports deferred-mapping HIP
                                        ///< arrays.
  ...
  size_t maxSharedMemoryPerMultiProcessor; ///< Max shared memory per CU (HIP
                                           ///< only).
  int clockInstructionRate; ///< Frequency (kHz) of the device-side clock*
                            ///< instructions.
  hipDeviceArch_t arch;     ///< Architectural feature flags (HIP only).
  unsigned int
      *hdpMemFlushCntl; ///< Address of HDP_MEM_COHERENCY_FLUSH_CNTL register.
  unsigned int
      *hdpRegFlushCntl; ///< Address of HDP_REG_COHERENCY_FLUSH_CNTL register.
  int cooperativeMultiDeviceUnmatchedFunc; ///< Multi-device cooperative launch,
                                           ///< unmatched funcs.
  int cooperativeMultiDeviceUnmatchedGridDim; ///< ...unmatched grid dimensions.
  int cooperativeMultiDeviceUnmatchedBlockDim;  ///< ...unmatched block
                                                ///< dimensions.
  int cooperativeMultiDeviceUnmatchedSharedMem; ///< ...unmatched shared
                                                ///< memories.
  int isLargeBar;   ///< 1 if large PCI bar device, else 0.
  int asicRevision; ///< Revision of the GPU in this device.
} hipDeviceProp_tR0600;

// The properties RPC transfers the fixed ROCm 6 x64 ABI layout. Fail at build
// time instead of silently changing the wire contract on an unsupported ABI.
static_assert(sizeof(void *) == 8, "HIP R0600 RPC requires a 64-bit ABI");
static_assert(sizeof(size_t) == 8, "HIP R0600 RPC requires 64-bit size_t");
static_assert(sizeof(hipDeviceProp_tR0600) == 1472,
              "unexpected hipDeviceProp_tR0600 size");
static_assert(alignof(hipDeviceProp_tR0600) == 8,
              "unexpected hipDeviceProp_tR0600 alignment");
static_assert(offsetof(hipDeviceProp_tR0600, gcnArchName) == 1160,
              "unexpected hipDeviceProp_tR0600 HIP extension offset");
static_assert(offsetof(hipDeviceProp_tR0600, hdpMemFlushCntl) == 1432,
              "unexpected hipDeviceProp_tR0600 pointer offset");
static_assert(offsetof(hipDeviceProp_tR0600, hdpRegFlushCntl) == 1440,
              "unexpected hipDeviceProp_tR0600 pointer offset");
static_assert(offsetof(hipDeviceProp_tR0600, asicRevision) == 1468,
              "unexpected hipDeviceProp_tR0600 tail offset");
