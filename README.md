...
Client images contain the CUDA driver, CUDA runtime, cuBLAS, cuBLASLt, cuFFT, cuDNN, cuRAND, NVML, and HIP shims, their runtime dependencies,
...
at `build/libcudart.so.<major>`, the cuBLAS, cuBLASLt, cuFFT and cuRAND shims
at `build/libcublas.so.<major>`, `build/libcublasLt.so.<major>`,
`build/libcufft.so.<major>` and `build/libcurand.so.<major>` (when the
toolkit's library headers are present),
...
`cudnn*` and `curand*` calls on the driver shim's connections, so all of them must come from
