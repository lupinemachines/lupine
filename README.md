...
Client images contain the CUDA driver, CUDA runtime, cuBLAS, NVML, and HIP shims, their runtime dependencies,
...
at `build/libcudart.so.<major>`, the cuBLAS shim at `build/libcublas.so.<major>`
(when the toolkit's cuBLAS headers are present), the NVML shim at
`build/libnvidia-ml.so.1`, the HIP shim at `build/libamdhip64.so.1`, and the
server at `build/lupine_driver_server`. The runtime and cuBLAS shims cover their
whole APIs: they forward `cuda*` and `cublas*` calls on the driver shim's
connections, so all three must come from the same build.
