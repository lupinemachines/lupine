...
Client images contain the CUDA driver, CUDA runtime, cuBLAS, cuFFT, NVML, and HIP shims, their runtime dependencies,
...
at `build/libcudart.so.<major>`, the cuBLAS and cuFFT shims at
`build/libcublas.so.<major>` and `build/libcufft.so.<major>` (when the
toolkit's library headers are present), the NVML shim at
`build/libnvidia-ml.so.1`, the HIP shim at `build/libamdhip64.so.1`, and the
server at `build/lupine_driver_server`. The runtime, cuBLAS and cuFFT shims
cover their whole APIs: they forward `cuda*`, `cublas*` and `cufft*` calls on
the driver shim's connections, so all four must come from the same build.
