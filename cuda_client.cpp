...
static libcuckoo::cuckoohash_map<lupine_param_layout_key, size_t,
                                 lupine_param_layout_key_hash> &
lupine_param_layout_count_cache() {
  static auto *cache =
      new libcuckoo::cuckoohash_map<lupine_param_layout_key, size_t,
                                    lupine_param_layout_key_hash>();
  return *cache;
}
...
extern "C" CUresult cuInit(unsigned int flags) {
  ...
  if (lupine_local_cuda_available()) {
    CUresult result = lupine_call_real_cuda_fn("cuInit", flags);
...
extern "C" CUresult cuDeviceGetP2PAttribute(int *value,
  ...
  if (lupine_route_is_local(src_route)) {
    return lupine_call_real_cuda_fn("cuDeviceGetP2PAttribute", value, attrib,
                                    srcDevice, dstDevice);
  }
...
extern "C" CUresult cuDeviceCanAccessPeer(int *canAccessPeer, CUdevice dev,
  ...
  if (lupine_route_is_local(route)) {
    ...
    return lupine_call_real_cuda_fn("cuDeviceCanAccessPeer", canAccessPeer, dev,
                                    peer);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  if (conn == nullptr) {
  ...
  CUresult result = CUDA_ERROR_DEVICE_UNAVAILABLE;
  if (lupine_prepare_rpc(conn) < 0 ||
      rpc_write_start_request(conn, RPC_cuDeviceCanAccessPeer) < 0 ||
...
extern "C" CUresult cuCtxEnablePeerAccess(CUcontext peerContext,
  ...
  if (lupine_route_is_local(current_route)) {
    return lupine_call_real_cuda_fn("cuCtxEnablePeerAccess", peerContext,
                                    flags);
  }
...
extern "C" CUresult cuCtxDisablePeerAccess(CUcontext peerContext) {
  ...
  if (lupine_route_is_local(current_route)) {
    return lupine_call_real_cuda_fn("cuCtxDisablePeerAccess", peerContext);
  }
  conn_t *conn = lupine_route_remote_conn(current_route);
  CUresult result = CUDA_ERROR_DEVICE_UNAVAILABLE;
  if (lupine_prepare_rpc(conn) < 0 ||
...
static void lupine_cache_library_kernel(CUkernel kernel, CUlibrary library,
  ...
  record.kernels_by_route[lupine_route_identity(route)] = kernel;
}
...
extern "C" CUresult cuLibraryGetKernel(CUkernel *pKernel, CUlibrary library,
  ...
  if (lupine_route_is_local(route)) {
    return_value =
        lupine_call_real_cuda_fn("cuLibraryGetKernel", pKernel, library, name);
  ...
  if (lupine_library_kernel_names().find(
          lupine_library_kernel_name_key{library, std::string(name)}, cached)) {
    *pKernel = cached;
    return CUDA_SUCCESS;
  }
  conn_t *conn = lupine_route_remote_conn(route);
...
extern "C" CUresult lupine_record_library_kernel(CUkernel kernel,
  ...
  lupine_cache_library_kernel(kernel, library, name, route);
  return CUDA_SUCCESS;
}
...
static void lupine_prefetch_function_param_layout(CUfunction function,
  ...
  for (;; ++index) {
    ...
    lupine_param_info_cache().insert_or_assign(
        lupine_param_info_key{handle, index, false},
        lupine_param_info_value{offset, size});
  }
  ...
  if (result == CUDA_ERROR_INVALID_VALUE) {
    lupine_param_layout_count_cache().insert_or_assign(
        lupine_param_layout_key{handle, false}, index);
  }
}
...
extern "C" CUresult cuModuleGetFunction(CUfunction *function, CUmodule module,
  ...
  if (lupine_route_is_local(route)) {
    result =
        lupine_call_real_cuda_fn("cuModuleGetFunction", function, module, name);
    if (result == CUDA_SUCCESS) {
...
static int lupine_read_library(conn_t *conn, CUlibrary library);
...
extern "C" CUresult cuModuleLoad(CUmodule *module, const char *fname) {
  ...
  lupine_route route = lupine_route_for_default();
  if (lupine_route_is_local(route)) {
    CUresult result = lupine_call_real_cuda_fn("cuModuleLoad", module, fname);
  ...
  bool failed = lupine_prepare_rpc(conn) < 0 ||
  ...
  *library = nullptr;
  ...
  if (lupine_route_is_local(route)) {
    ...
    if (lupine_prepare_rpc(conn) < 0 ||
        ...
        rpc_read(conn, &loaded, sizeof(loaded)) < 0 ||
        rpc_read(conn, &result, sizeof(result)) < 0 ||
        (result == CUDA_SUCCESS && lupine_read_library(conn, loaded) < 0) ||
        rpc_read_end(conn) < 0) {
      return CUDA_ERROR_DEVICE_UNAVAILABLE;
    }
  }
...
static CUresult lupine_resolve_library_kernel_for_route(CUfunction function,
  ...
  *resolved = function;
  ...
  CUkernel kernel = nullptr;
  if (lupine_route_is_local(route)) {
    result = lupine_call_real_cuda_fn("cuLibraryGetKernel", &kernel, library,
                                      record.name.c_str());
  } else {
    conn_t *conn = lupine_route_remote_conn(route);
...
static void lupine_prefill_device_snapshot(conn_t *conn) {
  ...
  CUresult result = CUDA_ERROR_UNKNOWN;
  if (lupine_prepare_rpc(conn) < 0 ||
      rpc_write_start_request(conn, LUPINE_RPC_lupineDeviceSnapshot) < 0 ||
      rpc_wait_for_response(conn) < 0 ||
      rpc_read(conn, &result, sizeof(result)) < 0) {
    return;
  }
...
extern "C" CUresult cuDeviceGetAttribute(int *pi, CUdevice_attribute attrib,
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuDeviceGetAttribute", pi, attrib,
                                    remote_dev);
  }
...
extern "C" CUresult cuDeviceGetName(char *name, int len, CUdevice dev) {
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuDeviceGetName", name, len, remote_dev);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  if (len > 0) {
    lupine_prefill_device_snapshot(conn);
...
extern "C" CUresult cuDeviceGetUuid_v2(CUuuid *uuid, CUdevice dev) {
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuDeviceGetUuid_v2", uuid, remote_dev);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  lupine_prefill_device_snapshot(conn);
  lupine_device_snapshot_info info;
...
extern "C" CUresult cuDeviceTotalMem_v2(size_t *bytes, CUdevice dev) {
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuDeviceTotalMem_v2", bytes, remote_dev);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  lupine_prefill_device_snapshot(conn);
  lupine_device_snapshot_info info;
...
extern "C" CUresult cuDevicePrimaryCtxSetFlags_v2(CUdevice dev,
  ...
  CUresult return_value;
  if (lupine_route_is_local(route)) {
    return_value = lupine_call_real_cuda_fn("cuDevicePrimaryCtxSetFlags_v2",
                                            remote_dev, flags);
    return return_value;
  }
  conn_t *conn = lupine_route_remote_conn(route);
...
cuDevicePrimaryCtxGetState(CUdevice dev, unsigned int *flags, int *active) {
  ...
  CUresult return_value;
  if (lupine_route_is_local(route)) {
    return_value = lupine_call_real_cuda_fn("cuDevicePrimaryCtxGetState",
                                            remote_dev, flags, active);
    return return_value;
  }
...
extern "C" CUresult cuDevicePrimaryCtxReset_v2(CUdevice dev) {
  ...
  if (lupine_route_is_local(route)) {
    return_value =
        lupine_call_real_cuda_fn("cuDevicePrimaryCtxReset_v2", remote_dev);
    if (return_value == CUDA_SUCCESS) {
      lupine_invalidate_current_context_cache();
    }
    return return_value;
...
static CUresult lupine_cuGetParamInfo_cached(uintptr_t handle,
  ...
  if (lupine_route_is_local(route)) {
    if (kernel) {
      return lupine_call_real_cuda_fn<CUDA_ERROR_NOT_SUPPORTED>(
          "cuKernelGetParamInfo", reinterpret_cast<CUkernel>(handle),
          param_index, param_offset, param_size);
    }
    return lupine_call_real_cuda_fn<CUDA_ERROR_NOT_SUPPORTED>(
        "cuFuncGetParamInfo", function, param_index, param_offset, param_size);
  }
  ...
  if (lupine_prepare_rpc(conn) < 0 || rpc_write_start_request(conn, rpc) < 0 ||
  ...
  if (result == CUDA_SUCCESS) {
    lupine_param_info_cache().insert_or_assign(
        key, lupine_param_info_value{offset, size});
    *param_offset = offset;
    *param_size = size;
  }
  return result;
...
extern "C" CUresult cuKernelGetFunction(CUfunction *pFunc, CUkernel kernel) {
  ...
  if (lupine_route_is_local(route)) {
    CUresult result =
        lupine_call_real_cuda_fn("cuKernelGetFunction", pFunc, route_kernel);
    if (result == CUDA_SUCCESS) {
...
extern "C" CUresult cuKernelGetAttribute(int *pi, CUfunction_attribute attrib,
  ...
  CUresult return_value;
  if (lupine_route_is_local(route)) {
    return_value = lupine_call_real_cuda_fn("cuKernelGetAttribute", pi, attrib,
                                            kernel, dev);
    return return_value;
  }
...
extern "C" CUresult cuFuncGetAttribute(int *pi, CUfunction_attribute attrib,
  ...
  if (lupine_route_is_local(route)) {
    return_value =
        lupine_call_real_cuda_fn("cuFuncGetAttribute", pi, attrib, translated);
    return return_value;
  }
...
static CUresult lupine_cuOccupancy_cached(int *numBlocks, CUfunction func,
  ...
  if (lupine_route_is_local(route)) {
    ...
    if (with_flags) {
      return lupine_call_real_cuda_fn(symbol, numBlocks, translated, blockSize,
  ...
  if (lupine_prepare_rpc(conn) < 0 ||
      rpc_write_start_request(conn, opcode) < 0 ||
...
static const void *lupine_remote_private_export_table(const CUuuid *uuid) {
  ...
  if (lupine_prepare_rpc(conn) < 0 ||
      rpc_write_start_request(conn, LUPINE_RPC_cuGetExportTableMetadata) < 0 ||
...
static CUresult lupine_error_string_lookup(int rpc_op, const char *symbol,
  ...
  *pStr = nullptr;
  ...
  lupine_route route = lupine_route_for_default();
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn(symbol, error, pStr);
  }
  ...
  uint32_t length = 0;
  if (lupine_prepare_rpc(conn) < 0 ||
      rpc_write_start_request(conn, rpc_op) < 0 ||
...
extern "C" CUresult cuEventDestroy_v2(CUevent hEvent) {
  ...
  CUresult result = CUDA_ERROR_DEVICE_UNAVAILABLE;
  if (lupine_route_is_local(route)) {
    result = lupine_call_real_cuda_fn("cuEventDestroy_v2", hEvent);
    if (result == CUDA_SUCCESS) {
...
extern "C" CUresult cuMemPoolSetAttribute(CUmemoryPool pool,
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuMemPoolSetAttribute", pool, attr, value);
  }
...
CUresult cuMemExportToShareableHandle(void *shareableHandle,
  ...
  lupine_route route = lupine_route_for_default();
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuMemExportToShareableHandle",
...
cuMemImportFromShareableHandle(CUmemGenericAllocationHandle *handle,
  ...
  lupine_route route = lupine_route_for_default();
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuMemImportFromShareableHandle", handle,
...
CUresult cuMemPoolExportToShareableHandle(void *handle_out, CUmemoryPool pool,
  ...
  lupine_route route = lupine_route_for_memory_pool(pool);
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuMemPoolExportToShareableHandle",
...
cuMemPoolImportFromShareableHandle(CUmemoryPool *pool_out, void *handle,
  ...
  if (lupine_route_is_local(route)) {
    return_value =
        lupine_call_real_cuda_fn("cuMemPoolImportFromShareableHandle", pool_out,
...
extern "C" CUresult cuCtxGetDevice(CUdevice *device) {
  ...
  lupine_route route = lupine_route_for_current_context();
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuCtxGetDevice", device);
  }
...
extern "C" CUresult cuCtxGetDevice_v2(CUdevice *device, CUcontext ctx) {
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuCtxGetDevice_v2", device, ctx);
  }
  ...
  CUresult return_value;
  if (lupine_prepare_rpc(conn) < 0 ||
      rpc_write_start_request(conn, RPC_cuCtxGetDevice_v2) < 0 ||
      rpc_write(conn, &ctx, sizeof(ctx)) < 0 ||
      rpc_wait_for_response(conn) < 0 ||
      rpc_read(conn, &remote_device, sizeof(remote_device)) < 0 ||
      rpc_read(conn, &return_value, sizeof(return_value)) < 0 ||
...
extern "C" CUresult cuMemPoolGetAttribute(CUmemoryPool pool,
  ...
  lupine_route route = lupine_route_for_memory_pool(pool);
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuMemPoolGetAttribute", pool, attr, value);
  }
...
extern "C" CUresult cuMemPrefetchAsync(CUdeviceptr devPtr, size_t count,
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuMemPrefetchAsync", devPtr, count,
                                    route_device, hStream);
  }
  if (managed_alias) {
    return CUDA_SUCCESS;
  }
...
extern "C" CUresult cuMemAdvise(CUdeviceptr devPtr, size_t count,
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuMemAdvise", devPtr, count, advice,
                                    route_device);
  }
...
extern "C" CUresult cuMemPrefetchAsync_v2(CUdeviceptr devPtr, size_t count,
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuMemPrefetchAsync_v2", devPtr, count,
                                    route_location, flags, hStream);
  }
  if (managed_alias) {
    return CUDA_SUCCESS;
  }
...
extern "C" CUresult cuMemAdvise_v2(CUdeviceptr devPtr, size_t count,
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuMemAdvise_v2", devPtr, count, advice,
                                    route_location);
  }
  if (managed_alias) {
    return CUDA_SUCCESS;
...
extern "C" CUresult cuCtxGetStreamPriorityRange(int *leastPriority,
  ...
  lupine_route route = lupine_route_for_default();
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuCtxGetStreamPriorityRange",
...
extern "C" CUresult cuLinkCreate_v2(unsigned int numOptions,
  ...
  lupine_route route = lupine_route_for_default();
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuLinkCreate_v2", numOptions, options,
...
extern "C" CUresult cuLinkAddData_v2(CUlinkState state, CUjitInputType type,
  ...
  lupine_route route = lupine_route_for_default();
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuLinkAddData_v2", state, type, data, size,
                                    name, numOptions, options, optionValues);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  CUresult return_value;
...
extern "C" CUresult cuLinkAddFile_v2(CUlinkState state, CUjitInputType type,
  ...
  lupine_route route = lupine_route_for_default();
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuLinkAddFile_v2", state, type, path,
...
extern "C" CUresult cuLinkComplete(CUlinkState state, void **cubinOut,
  ...
  lupine_route route = lupine_route_for_default();
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuLinkComplete", state, cubinOut, sizeOut);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  CUresult return_value;
  size_t cubin_size = 0;
...
extern "C" CUresult cuLinkDestroy(CUlinkState state) {
  lupine_route route = lupine_route_for_default();
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuLinkDestroy", state);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  CUresult return_value;
  if (lupine_prepare_rpc(conn) < 0 ||
...
extern "C" int lupine_read_deferred_dtoh_copies(conn_t *conn) {
  ...
  if (rpc_read_buffer(conn, &copy_count, sizeof(copy_count)) < 0) {
    return -1;
  }
...
extern "C" CUresult cuStreamWaitEvent(CUstream hStream, CUevent hEvent,
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuStreamWaitEvent", hStream, hEvent,
                                    Flags);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  CUresult result = CUDA_ERROR_DEVICE_UNAVAILABLE;
...
extern "C" CUresult cuCtxCreate_v2(CUcontext *pctx, unsigned int flags,
  ...
  if (lupine_route_is_local(route)) {
    CUresult result =
        lupine_call_real_cuda_fn("cuCtxCreate_v2", pctx, flags, dev);
    if (result == CUDA_SUCCESS) {
...
static CUresult lupine_occupancy_max_potential_block_size(
  ...
  if (lupine_route_is_local(route)) {
    if (with_flags) {
      return lupine_call_real_cuda_fn(symbol, minGridSize, blockSize,
                                      translated, blockSizeToDynamicSMemSize,
                                      dynamicSMemSize, blockSizeLimit, flags);
    }
    return lupine_call_real_cuda_fn(symbol, minGridSize, blockSize, translated,
  ...
  if (lupine_prepare_rpc(conn) < 0 || rpc_write_start_request(conn, op) < 0 ||
      rpc_write(conn, &translated, sizeof(translated)) < 0 ||
      rpc_write(conn, &dynamicSMemSize, sizeof(dynamicSMemSize)) < 0 ||
...
// Field-for-field inverse of lupine_write_library. Populate the caches as
// records arrive, without a packed buffer or intermediate table. Only local
// cache operations are allowed here: the load response is still being read.
static int lupine_read_library(conn_t *conn, CUlibrary library) {
  CUresult result = CUDA_ERROR_UNKNOWN;
  CUdevice device = -1;
  uint32_t count = 0;
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    uint32_t param_count = 0;
    if (rpc_read(conn, name.data(), name_length) < 0 || name.back() != '\0' ||
        rpc_read(conn, &kernel, sizeof(kernel)) < 0 || kernel == nullptr ||
        rpc_read(conn, &function, sizeof(function)) < 0 ||
        rpc_read(conn, &param_count, sizeof(param_count)) < 0 ||
        param_count > 64 * 1024) {
      return -1;
    ...
    for (uint32_t index = 0; index < param_count; ++index) {
      ...
      lupine_param_info_cache().insert_or_assign(
          lupine_param_info_key{reinterpret_cast<uintptr_t>(kernel), index,
                                true},
          param);
      if (function != nullptr) {
        lupine_param_info_cache().insert_or_assign(
            lupine_param_info_key{reinterpret_cast<uintptr_t>(function), index,
                                  false},
            param);
      }
    }
    ...
    if (function != nullptr) {
      lupine_note_function_owner_route(function, route);
      lupine_param_layout_count_cache().insert_or_assign(
          lupine_param_layout_key{reinterpret_cast<uintptr_t>(function), false},
          param_count);
      lupine_kernel_function_cache().insert_or_assign(
          lupine_kernel_function_key{lupine_route_identity(route), context,
                                     kernel},
          function);
    }
    lupine_library_kernel_names().insert_or_assign(
        lupine_library_kernel_name_key{library, name}, kernel);
    lupine_note_function_owner_route(reinterpret_cast<CUfunction>(kernel),
                                     route);
    lupine_cache_library_kernel(kernel, library, std::move(name), route);
    if (function != nullptr) {
...
// The request tail after the image: JIT options, then library options with an
// absent value array encoded as each option's complement. The same bytes key
// the profile, so a pointer-valued option that differs between runs misses.
...
// the previous run of this command line loaded, requested from the server's
// cache in one round trip. Entries are handed out once, so every handle cudart
// receives is distinct and cuLibraryUnload keeps its native meaning.
...
// `python eval.py` keep separate load orders.
static std::filesystem::path lupine_library_profile_path() {
  ...
  if (key.empty()) {
    ...
    if (!argv.empty()) {
      key = lupine_library_hash_name(XXH64(argv.data(), argv.size(), 0));
    }
  }
...
static void lupine_library_batch_main(conn_t *conn, CUcontext context) {
  ...
  lupine_current_context = context;
  auto count = static_cast<uint32_t>(state.entries.size());
  bool ok =
  ...
  for (const auto &entry : state.entries) {
    ok = ok && rpc_write(conn, &entry.hash, sizeof(entry.hash)) >= 0 &&
         rpc_write(conn, &entry.kind, sizeof(entry.kind)) >= 0 &&
         rpc_write(conn, entry.options.data(), entry.options.size()) >= 0;
  }
  ok = ok && rpc_wait_for_response(conn) >= 0;
  uint32_t cached = 0;
  for (auto &entry : state.entries) {
    if (!ok ||
        lupine_read_library_load_response(conn, 0, nullptr, nullptr,
                                          &entry.library, &entry.result) < 0) {
      ok = false;
      entry.result = CUDA_ERROR_UNKNOWN;
    }
    cached += entry.result == CUDA_SUCCESS;
  }
  ok = ok && rpc_read_end(conn) >= 0;
  LUPINE_TRACE_LOG("LUPINE library batch: " << cached << "/" << count
                                            << " cached, ok=" << ok);
...
static void lupine_library_batch_start(lupine_route route) {
  ...
  std::string data((std::istreambuf_iterator<char>(in)),
                   std::istreambuf_iterator<char>());
  for (size_t at = 0; at + 16 <= data.size() && state.entries.size() < 4096;) {
    lupine_library_batch_entry entry;
    uint32_t size = 0;
    std::memcpy(&entry.hash, &data[at], sizeof(entry.hash));
    std::memcpy(&entry.kind, &data[at + 8], sizeof(entry.kind));
    std::memcpy(&size, &data[at + 12], sizeof(size));
    at += 16;
    if (size < 8 || size > data.size() - at) {
      break;
    }
    entry.options.assign(&data[at], &data[at] + size);
    at += size;
...
static bool
lupine_library_batch_claim(lupine_route route, uint64_t hash, uint32_t kind,
                           const std::vector<unsigned char> &options,
                           CUlibrary *library) {
  auto &state = lupine_library_batch();
...
static void
lupine_library_profile_record(lupine_route route, uint64_t hash, uint32_t kind,
                              const std::vector<unsigned char> &options) {
  auto &state = lupine_library_batch();
  std::lock_guard<std::mutex> lock(state.mutex);
  unsigned int jit_options = 0;
  std::memcpy(&jit_options, options.data(), sizeof(jit_options));
  if (state.profile.empty() || jit_options != 0 ||
      lupine_route_identity(route) != state.route_id) {
    return;
  }
  auto size = static_cast<uint32_t>(options.size());
  auto append = [&](const void *data, size_t length) {
    state.lines.append(static_cast<const char *>(data), length);
  };
  append(&hash, sizeof(hash));
  append(&kind, sizeof(kind));
  append(&size, sizeof(size));
  append(options.data(), size);
...
cuLibraryLoadData(CUlibrary *library, const void *code,
  ...
  conn_t *conn = lupine_route_remote_conn(route);
  std::vector<unsigned char> options = lupine_pack_library_load_options(
      jitOptions, jitOptionsValues, numJitOptions, libraryOptions,
      libraryOptionValues, numLibraryOptions);
  uint64_t hash = XXH64(image_bytes.data(), image_bytes.size(), 0);
  size_t image_size = image_bytes.size();
  lupine_library_batch_start(route);
  CUresult return_value = CUDA_SUCCESS;
  if (!lupine_library_batch_claim(route, hash, kind, options, library) &&
      (lupine_prepare_rpc(conn) < 0 ||
       rpc_write_start_request(conn, RPC_cuLibraryLoadData) < 0 ||
       rpc_write(conn, &kind, sizeof(kind)) < 0 ||
       rpc_write(conn, &image_size, sizeof(image_size)) < 0 ||
       rpc_write(conn, image_bytes.data(), image_size) < 0 ||
       rpc_write(conn, options.data(), options.size()) < 0 ||
       rpc_wait_for_response(conn) < 0 ||
  ...
  if (return_value == CUDA_SUCCESS) {
    ...
    lupine_record_library_image(*library, lupine_remote_route_for_conn(conn),
                                kind, image_bytes.data(), image_bytes.size(),
                                code);
    lupine_library_profile_record(route, hash, kind, options);
  }
  return return_value;
}
...
lupine_kernel_param_cursors(void *const *kernel_params,
  ...
  for (size_t i = 0; i < sizes.size(); ++i) {
    cursors.push_back(rpc_write_cursor(kernel_params[i], sizes[i]));
  }
  return cursors;
}
...
static CUkernel lupine_client_kernel_for_remote(CUkernel remote) {
  if (remote == nullptr) {
    return nullptr;
  }
  std::lock_guard<std::mutex> lock(lupine_library_kernel_mutex());
...
cuGraphKernelNodeGetParams_v2(CUgraphNode hNode,
  ...
  if (param_info_result != CUDA_SUCCESS) {
...
cuGraphAddKernelNode_v2(CUgraphNode *phGraphNode, CUgraph hGraph,
  ...
  if (lupine_route_is_local(route)) {
    local_result =
        lupine_call_real_cuda_fn("cuGraphAddKernelNode_v2", phGraphNode, hGraph,
...
cuGraphAddMemcpyNode(CUgraphNode *phGraphNode, CUgraph hGraph,
  ...
  CUresult local_result = CUDA_ERROR_DEVICE_UNAVAILABLE;
  if (lupine_route_is_local(route)) {
    local_result = lupine_call_real_cuda_fn("cuGraphAddMemcpyNode", phGraphNode,
                                            hGraph, dependencies,
...
cuGraphExecKernelNodeSetParams_v2(CUgraphExec hGraphExec, CUgraphNode hNode,
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuGraphExecKernelNodeSetParams_v2",
...
cuGraphAddMemsetNode(CUgraphNode *phGraphNode, CUgraph hGraph,
  ...
  CUresult local_result = CUDA_ERROR_DEVICE_UNAVAILABLE;
  if (lupine_route_is_local(route)) {
    local_result = lupine_call_real_cuda_fn("cuGraphAddMemsetNode", phGraphNode,
                                            hGraph, dependencies,
...
cuGraphAddHostNode(CUgraphNode *phGraphNode, CUgraph hGraph,
  ...
  if (lupine_route_is_local(route)) {
    local_result =
        lupine_call_real_cuda_fn("cuGraphAddHostNode", phGraphNode, hGraph,
                                 dependencies, numDependencies, nodeParams);
    if (local_result == CUDA_SUCCESS) {
      lupine_note_graph_node_owner_route(*phGraphNode, route);
    }
...
extern "C" CUresult cuGraphConditionalHandleCreate(
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuGraphConditionalHandleCreate",
                                    pHandle_out, hGraph, ctx,
...
extern "C" CUresult cuGraphAddNode_v2(CUgraphNode *phGraphNode, CUgraph hGraph,
  ...
  CUresult local_result = CUDA_ERROR_DEVICE_UNAVAILABLE;
  if (lupine_route_is_local(route)) {
    local_result = lupine_call_real_cuda_fn(
        "cuGraphAddNode_v2", phGraphNode, hGraph, dependencies, dependencyData,
        numDependencies, nodeParams);
    if (local_result == CUDA_SUCCESS) {
      lupine_note_graph_node_owner_route(*phGraphNode, route);
...
#if CUDA_VERSION >= 12020
  // Reuse the type-specific wrappers for variants with nested host pointers
  // or callbacks. They already provide the process-boundary serialization
  // that a shallow CUgraphNodeParams copy cannot.
  switch (nodeParams->type) {
  case CU_GRAPH_NODE_TYPE_MEMCPY:
    return cuGraphAddMemcpyNode(phGraphNode, hGraph, dependencies,
                                numDependencies, &nodeParams->memcpy.copyParams,
                                nodeParams->memcpy.copyCtx);
  case CU_GRAPH_NODE_TYPE_MEMSET: {
    CUDA_MEMSET_NODE_PARAMS params{};
    params.dst = nodeParams->memset.dst;
    params.pitch = nodeParams->memset.pitch;
    params.value = nodeParams->memset.value;
    params.elementSize = nodeParams->memset.elementSize;
    params.width = nodeParams->memset.width;
    params.height = nodeParams->memset.height;
#if CUDA_VERSION >= 13020
    CUcontext ctx = nodeParams->memset.ctx;
#else
    CUcontext ctx = nullptr;
#endif
    return cuGraphAddMemsetNode(phGraphNode, hGraph, dependencies,
                                numDependencies, &params, ctx);
  }
  case CU_GRAPH_NODE_TYPE_HOST: {
    CUDA_HOST_NODE_PARAMS params{};
    params.fn = nodeParams->host.fn;
    params.userData = nodeParams->host.userData;
    return cuGraphAddHostNode(phGraphNode, hGraph, dependencies,
...
// CUDA 12.3 remaps these source names to generated edge-data v2 wrappers.
// Keep the literal legacy ABI symbols for older binaries.
...
cuGraphHostNodeGetParams(CUgraphNode hNode, CUDA_HOST_NODE_PARAMS *nodeParams) {
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuGraphHostNodeGetParams", hNode,
                                    nodeParams);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  CUresult return_value;
...
cuGraphHostNodeSetParams(CUgraphNode hNode,
  ...
  lupine_route route = lupine_route_for_graph_node(hNode);
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuGraphHostNodeSetParams", hNode,
                                    nodeParams);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  CUresult return_value;
...
cuGraphExecHostNodeSetParams(CUgraphExec hGraphExec, CUgraphNode hNode,
  ...
  lupine_route route = lupine_route_for_graph_exec(hGraphExec);
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuGraphExecHostNodeSetParams", hGraphExec,
                                    hNode, nodeParams);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  CUresult return_value;
...
extern "C" CUresult cuLaunchHostFunc(CUstream hStream, CUhostFn fn,
  ...
  lupine_route route = hStream == nullptr ? lupine_route_for_current_context()
                                          : lupine_route_for_stream(hStream);
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuLaunchHostFunc", hStream, fn, userData);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  CUresult return_value;
  if (lupine_prepare_rpc(conn) < 0 ||
...
extern "C" CUresult cuStreamAddCallback(CUstream hStream,
  ...
  lupine_route route = hStream == nullptr ? lupine_route_for_current_context()
                                          : lupine_route_for_stream(hStream);
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuStreamAddCallback", hStream, callback,
                                    userData, flags);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  CUresult return_value;
...
static CUresult lupine_cuStreamGetCaptureInfo(
...
#if CUDA_VERSION >= 12000
  {
    ...
    if (lupine_route_is_local(route)) {
      local_result = lupine_call_real_cuda_fn(
          "cuStreamGetCaptureInfo_v3", stream, captureStatus_out, id_out,
          graph_out, dependencies_out, edgeData_out, numDependencies_out);
      if (local_result == CUDA_SUCCESS) {
  ...
  {
    if (lupine_route_is_local(route)) {
      return lupine_call_real_cuda_fn("cuStreamGetCaptureInfo", stream,
  ...
  CUresult return_value;
  if (lupine_prepare_rpc(conn) < 0 ||
      rpc_write_start_request(conn, LUPINE_RPC_cuStreamGetCaptureInfo_v3) < 0 ||
...
extern "C" CUresult cuStreamIsCapturing(CUstream hStream,
  ...
  CUresult return_value;
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuStreamIsCapturing", hStream,
                                    captureStatus);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  if (lupine_prepare_rpc(conn) < 0 ||
...
extern "C" CUresult cuStreamBeginCapture_v2(CUstream hStream,
  ...
  CUresult return_value;
  if (lupine_route_is_local(route)) {
    return capture_guard.complete(
        lupine_call_real_cuda_fn("cuStreamBeginCapture_v2", hStream, mode));
  }
  conn_t *conn = lupine_route_remote_conn(route);
  if (lupine_prepare_rpc(conn) < 0 ||
...
extern "C" CUresult cuStreamEndCapture(CUstream hStream, CUgraph *phGraph) {
  ...
  if (lupine_route_is_local(route)) {
    return_value =
        lupine_call_real_cuda_fn("cuStreamEndCapture", hStream, phGraph);
...
cuStreamBeginCaptureToGraph(CUstream hStream, CUgraph hGraph,
  ...
  CUresult return_value;
  if (lupine_route_is_local(route)) {
    return capture_guard.complete(lupine_call_real_cuda_fn(
        "cuStreamBeginCaptureToGraph", hStream, hGraph, dependencies,
        dependencyData, numDependencies, mode));
  }

  conn_t *conn = lupine_route_remote_conn(route);
  if (lupine_prepare_rpc(conn) < 0 ||
      rpc_write_start_request(conn, LUPINE_RPC_cuStreamBeginCaptureToGraph) <
          0 ||
      rpc_write(conn, &hStream, sizeof(hStream)) < 0 ||
      rpc_write(conn, &hGraph, sizeof(hGraph)) < 0 ||
      lupine_queue_graph_dependencies(conn, dependencies, &numDependencies) !=
...
extern "C" CUresult cuStreamUpdateCaptureDependencies_v2(
  ...
  CUresult return_value;
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuStreamUpdateCaptureDependencies_v2",
                                    hStream, dependencies, dependencyData,
                                    numDependencies, flags);
  }

  conn_t *conn = lupine_route_remote_conn(route);
  if (lupine_prepare_rpc(conn) < 0 ||
      rpc_write_start_request(conn, RPC_cuStreamUpdateCaptureDependencies_v2) <
          0 ||
      rpc_write(conn, &hStream, sizeof(hStream)) < 0 ||
      lupine_queue_graph_dependencies(conn, dependencies, &numDependencies) !=
          CUDA_SUCCESS ||
...
extern "C" CUresult cuKernelGetLibrary(CUlibrary *pLib, CUkernel kernel) {
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn("cuKernelGetLibrary", pLib, kernel);
  }
  conn_t *conn = lupine_route_remote_conn(route);
  CUresult return_value;
  if (lupine_prepare_rpc(conn) < 0 ||
...
extern "C" CUresult cuTensorMapEncodeTiled(
  ...
  if (lupine_route_is_local(route)) {
    return lupine_call_real_cuda_fn(
        "cuTensorMapEncodeTiled", tensorMap, tensorDataType, tensorRank,
...
CUresult cuGetProcAddress_v2(const char *symbol, void **pfn, int cudaVersion,
  ...
  auto resolve_graph_query = [&](const char *candidate, void *legacy,
                                 void *edge_data) {
    if (strcmp(symbol, candidate) != 0) {
      return false;
    }
    *pfn = cudaVersion >= 12030 ? edge_data : legacy;
    if (symbolStatus != nullptr) {
      *symbolStatus = CU_GET_PROC_ADDRESS_SUCCESS;
    }
    return true;
  };
  if (resolve_graph_query("cuGraphGetEdges",
                          reinterpret_cast<void *>(&cuGraphGetEdges),
                          reinterpret_cast<void *>(&cuGraphGetEdges_v2)) ||
      resolve_graph_query(
          "cuGraphNodeGetDependencies",
          reinterpret_cast<void *>(&cuGraphNodeGetDependencies),
          reinterpret_cast<void *>(&cuGraphNodeGetDependencies_v2)) ||
      resolve_graph_query(
          "cuGraphNodeGetDependentNodes",
          reinterpret_cast<void *>(&cuGraphNodeGetDependentNodes),
          reinterpret_cast<void *>(&cuGraphNodeGetDependentNodes_v2))) {
    return CUDA_SUCCESS;
  }
