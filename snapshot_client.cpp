...
extern "C" CUresult lupine_snapshot_create(char *id, size_t id_len,
                                           unsigned int flags) {
  if (id == nullptr || id_len < LUPINE_SNAPSHOT_ID_BUFFER_BYTES) {
    return CUDA_ERROR_INVALID_VALUE;
  }
  ...
  if (conn == nullptr ||
      rpc_write_start_request(conn, LUPINE_RPC_snapshot_create) < 0 ||
      rpc_write(conn, &flags, sizeof(flags)) < 0 ||
      rpc_wait_for_response(conn) < 0 ||
      rpc_read(conn, &returned_len, sizeof(returned_len)) < 0) {
    return CUDA_ERROR_DEVICE_UNAVAILABLE;
  }
  if (returned_len >= id_len) {
    rpc_drain(conn, returned_len);
    rpc_read(conn, &result, sizeof(result));
    rpc_read_end(conn);
    return CUDA_ERROR_INVALID_VALUE;
  }
  if (returned_len != 0 && rpc_read(conn, id, returned_len) < 0) {
    rpc_read_end(conn);
    return CUDA_ERROR_DEVICE_UNAVAILABLE;
  }
  id[returned_len] = '\0';
...
extern "C" CUresult lupine_snapshot_status(const char *id,
  ...
  CUresult result = CUDA_ERROR_DEVICE_UNAVAILABLE;
  if (conn == nullptr ||
      rpc_write_start_request(conn, LUPINE_RPC_snapshot_status) < 0 ||
...
extern "C" CUresult lupine_snapshot_load(const char *id, unsigned int flags) {
  if (!valid_client_snapshot_id(id)) {
    return CUDA_ERROR_INVALID_VALUE;
  }
  conn_t *conn = snapshot_conn();
  CUresult result = CUDA_ERROR_DEVICE_UNAVAILABLE;
  if (conn == nullptr ||
      rpc_write_start_request(conn, LUPINE_RPC_snapshot_load) < 0 ||
      write_snapshot_id(conn, id) != CUDA_SUCCESS ||
      rpc_write(conn, &flags, sizeof(flags)) < 0 ||
      rpc_wait_for_response(conn) < 0 ||
      rpc_read(conn, &result, sizeof(result)) < 0 || rpc_read_end(conn) < 0) {
...
extern "C" CUresult lupine_snapshot_delete(const char *id) {
  if (!valid_client_snapshot_id(id)) {
    return CUDA_ERROR_INVALID_VALUE;
  }
  conn_t *conn = snapshot_conn();
  CUresult result = CUDA_ERROR_DEVICE_UNAVAILABLE;
  if (conn == nullptr ||
      rpc_write_start_request(conn, LUPINE_RPC_snapshot_delete) < 0 ||
      write_snapshot_id(conn, id) != CUDA_SUCCESS ||
      rpc_wait_for_response(conn) < 0 ||
      rpc_read(conn, &result, sizeof(result)) < 0 || rpc_read_end(conn) < 0) {
    return CUDA_ERROR_DEVICE_UNAVAILABLE;
