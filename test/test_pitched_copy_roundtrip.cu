...
bool roundtrip_3d(size_t width, size_t height, size_t depth,
                  const char *label) {
  const size_t host_pitch = width + 64;
  const size_t host_height = height + 3;
  const size_t host_slice = host_pitch * host_height;
  std::vector<unsigned char> source(host_slice * depth);
  std::vector<unsigned char> readback(host_slice * depth, 0xEE);
  for (size_t slice = 0; slice < depth; ++slice) {
    for (size_t row = 0; row < host_height; ++row) {
      for (size_t column = 0; column < host_pitch; ++column) {
        source[slice * host_slice + row * host_pitch + column] =
            row < height && column < width
                ? static_cast<unsigned char>((slice * 17 + row * 31 + column) &
                                             0xff)
                : 0xAA;
      }
    }
  ...
  cudaPitchedPtr device = {};
  if (!check(cudaMalloc3D(&device, extent), "cudaMalloc3D")) {
    return false;
  }
  cudaMemcpy3DParms copy = {};
  copy.extent = extent;
  copy.srcPtr =
      make_cudaPitchedPtr(source.data(), host_pitch, width, host_height);
  copy.dstPtr = device;
  copy.kind = cudaMemcpyHostToDevice;
  bool ok = check(cudaMemcpy3D(&copy), "cudaMemcpy3D HtoD");
  if (ok) {
    copy.srcPtr = device;
    copy.dstPtr =
        make_cudaPitchedPtr(readback.data(), host_pitch, width, host_height);
    copy.kind = cudaMemcpyDeviceToHost;
    ok = check(cudaMemcpy3D(&copy), "cudaMemcpy3D DtoH");
  }
  if (ok) {
    for (size_t slice = 0; slice < depth && ok; ++slice) {
      for (size_t row = 0; row < host_height && ok; ++row) {
        for (size_t column = 0; column < host_pitch && ok; ++column) {
          unsigned char got =
              readback[slice * host_slice + row * host_pitch + column];
          unsigned char want =
              row < height && column < width
                  ? static_cast<unsigned char>(
                        (slice * 17 + row * 31 + column) & 0xff)
                  : 0xEE;
          if (got != want) {
            std::fprintf(stderr,
                         "FAIL: %s slice %zu row %zu column %zu: got %02x "
                         "want %02x\n",
                         label, slice, row, column, got, want);
            ok = false;
          }
        }
      }
  ...
  cudaFree(device.ptr);
  if (ok) {
    std::printf("ok: %s (%zu x %zu x %zu, host pitch %zu, height %zu)\n", label,
                width, height, depth, host_pitch, host_height);
  }
  return ok;
}
...
int main() {
  ...
  if (!roundtrip_2d(256, 64, "unframed 2D with padding") ||
      ...
      !roundtrip_3d(256, 32, 8, "unframed 3D with padding") ||
      !roundtrip_3d(2048, 64, 32, "framed 3D with padding")) {
    return 1;
  }
