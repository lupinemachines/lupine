...
cuda_sample_ci_patched=0

# Keep benchmark-sized upstream samples small enough for compliance CI while
# still building and running the sample code paths.
replace_cuda_sample_source() {
  ...
  if grep -Fq "$before" "$file"; then
    CUDA_SAMPLE_BEFORE="$before" CUDA_SAMPLE_AFTER="$after" \
      perl -0pi -e 's/\Q$ENV{CUDA_SAMPLE_BEFORE}\E/$ENV{CUDA_SAMPLE_AFTER}/' "$file"
    cuda_sample_ci_patched=1
    echo "patched CUDA sample source: ${file#$CUDA_SAMPLES_DIR/}" >&2
  fi
}

write_small_cusolverrf_matrix() {
  local file="$1"

  [[ -f "$file" ]] && return 0
  echo "writing CUDA sample CI input: ${file#$CUDA_SAMPLES_DIR/}" >&2
  awk '
    BEGIN {
      n = 20
      nodes = n * n
      ...
      print "% Generated by run_cuda_samples.sh for CI"
      print nodes, nodes, nnz
      for (y = 0; y < n; y++) {
        for (x = 0; x < n; x++) {
          id = y * n + x + 1
          print id, id, "4"
...
prepare_cuda_sample_ci_sources() {
  local sample_srcdir=""
  local selected=" $* "

  case "$selected" in
    *" HSOpticalFlow "*)
      sample_srcdir="$(resolve_sample_srcdir HSOpticalFlow || true)"
      if [[ -n "$sample_srcdir" ]]; then
        replace_cuda_sample_source \
          "$sample_srcdir/main.cpp" \
          "const int nSolverIters = 500;" \
          "const int nSolverIters = 20;"
      fi
      ;;
  esac
  ...
  case "$selected" in
    *" jacobiCudaGraphs "*)
      sample_srcdir="$(resolve_sample_srcdir jacobiCudaGraphs || true)"
      if [[ -n "$sample_srcdir" ]]; then
        replace_cuda_sample_source \
          "$sample_srcdir/jacobi.h" \
          "#define N_ROWS 512" \
          "#define N_ROWS 64"
      fi
      ;;
  ...
  case "$selected" in
    *" conjugateGradientPrecond "*)
      sample_srcdir="$(resolve_sample_srcdir conjugateGradientPrecond || true)"
      if [[ -n "$sample_srcdir" ]]; then
        replace_cuda_sample_source \
          "$sample_srcdir/main.cpp" \
  ...
  case "$selected" in
    *" watershedSegmentationNPP "*)
      sample_srcdir="$(resolve_sample_srcdir watershedSegmentationNPP || true)"
      if [[ -n "$sample_srcdir" ]]; then
        replace_cuda_sample_source \
          "$sample_srcdir/watershedSegmentationNPP.cpp" \
  ...
  case "$selected" in
    *" cuSolverRf "*)
      sample_srcdir="$(resolve_sample_srcdir cuSolverRf || true)"
      if [[ -n "$sample_srcdir" ]]; then
        write_small_cusolverrf_matrix "$sample_srcdir/lap2D_5pt_n20.mtx"
      fi
      ;;
  esac
}
...
sample_args() {
  ...
  case "$sample" in
    ...
    batchCUBLAS)
      ...
      ;;
    cuSolverRf)
      printf '%s\0' "-file=$(resolve_sample_srcdir cuSolverRf)/lap2D_5pt_n20.mtx"
      ;;
    eigenvalues)
...
prepare_cuda_sample_ci_sources "${samples[@]}"

needs_build=0
if [[ "$cuda_sample_ci_patched" == "1" ]]; then
  needs_build=1
elif [[ "$BUILD_SAMPLES" == "1" ]]; then
  needs_build=1
