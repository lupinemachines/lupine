...
// connection; the callback round trip must progress independently of a lane
// blocked in cuStreamSynchronize, or the connection deadlocks and dies on the
// dead-peer timer. Also checks that cuStreamSynchronize still waits for the
// client-side callback to finish.
#include <cuda.h>
  ...
  for (int t = 0; t < kThreads; ++t) {
    threads.emplace_back([&, t]() {
      ...
      for (int iteration = 0; iteration < kIterations; ++iteration) {
        ...
        check(
            cuMemcpyHtoDAsync(d_data, h_data, kElements * sizeof(int), stream),
            "cuMemcpyHtoDAsync", __LINE__);
        check(
            cuMemcpyDtoHAsync(h_data, d_data, kElements * sizeof(int), stream),
        ...
        for (size_t i = 0; i < kElements; i += 4096) {
          if (h_data[i] != base + static_cast<int>(i)) {
            std::fprintf(stderr,
                         "FAIL: data mismatch t=%d iteration=%d i=%zu\n", t,
                         iteration, i);
            failed.store(true, std::memory_order_release);
            return;
          }
